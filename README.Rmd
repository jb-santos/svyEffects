...
1. Support for (non-survey) weighted models and weighted models. While there 
...
methods to derive confidence intervals. 
  - *Note: You can actually already do this with `{svyEffects}` by creating a 
  survey design object with a weight of "1", but it would be good to avoid having
  to use that workaround. 
2. In-depth comparisons with Stata results.
3. Functions for calculating weighted model fit measures.
4. Support for using an alternative variance-covariance matrix using `{sandwich}`. 
...
I've seen). 
5. A second differences function to test for the significance of a two-way 
interaction.
6. (Possibly) Use of the delta method to calculate confidence intervals for AMEs 
probabilities to speed up computational time.
