...
- `modform`: the model formula used in the `svymultinom` call (in the form `modform = "y ~ x1 + x2 + x3"`); and
- `weightvar`: the name of the weight variable (in quotes).

*Note: I've submitted a pull request to `{svrepmisc}` to make some changes that will remove the need to specify these options. These changes should be incorporated within a few weeks, so check for updates.*
...
1. Support for "vanilla" `glm`, `polr`, and `multinom` models. While there 
